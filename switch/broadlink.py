...
def parse(packet, repeats=None):
    if packet.startswith('0000'):
        packet = _decode_pronto(packet)
    else:
        packet = _decode_base64(packet)

    if repeats:
        packet[1] = int(repeats)

    return packet


def _decode_base64(packet) -> bytearray:
    extra = len(packet) % 4
    if extra > 0:
...
def _decode_pronto(pronto) -> bytearray:
...
################################################################################
# Copied and patched from upstream c434ad6af57d758e2c36cd7f31eb2d4c6222a720
"""
...
class BroadlinkRMSwitch(SwitchDevice):
    ...
    def __init__(self, name, friendly_name, device, command_on, command_off):
        ...
        self._state = False
        self._command_on = parse(command_on) if command_on else None
        self._command_off = parse(command_off) if command_off else None
        self._device = device
